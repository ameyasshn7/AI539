...
class ParityLSTM(torch.nn.Module) :
    ...
    def forward(self, x, s):
      #TODO
      packed_input = nn.utils.rnn.pack_padded_sequence(x, s, batch_first=True, enforce_sorted=False)
      packed_output, (ht,ct) = self.lstm(packed_input)
      out, input_sizes = nn.utils.rnn.pad_packed_sequence(packed_output, batch_first=True)
      logits = self.fc(ht[-1])
      return logits
